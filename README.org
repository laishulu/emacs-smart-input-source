...
** Example: ~fcitx~
~fcitx~ is a input method popular among Chinese Linux users.
...
#+BEGIN_SRC lisp
(require 'subr-x)
(let ((ibus-path (executable-find "ibus")))
     (when ibus-path
       (setq smart-input-source-external-ism ibus-path)
       (setq smart-input-source-english-input-source "xkb:us::eng")
       (setq smart-input-source-other-input-source "rime")
       (setq smart-input-source-do-get-input-source
             (lambda()(string-trim (shell-command-to-string
                                    (concat ibus-path " engine")))))
       (setq smart-input-source-do-set-input-source
             (lambda(source)(string-trim (shell-command-to-string
                                    (concat ibus-path " engine " source)))))))
#+END_SRC
