...
| ~smart-input-source-auto-refresh-seconds~ | no     | Idle interval to auto refresh input source from OS | ~0.5~, ~nil~ to disable              |
|-------------------------------------------+--------+----------------------------------------------------+--------------------------------------|
...
**  How to make the package know the input source status precisely.
1. If your input source is switched from ~smart-input-source~, then everything
   should be OK naturally. And your ~smart-input-source-cursor-color-seconds~
   can even be set to ~nil~.
2. If your input source is switched from OS,
   ~smart-input-source-cursor-color-seconds~ should be small enough to timely
   detect the change of input source.
