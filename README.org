...
an even esier way. First configure with the following codes.
#+begin_src
(setq sis-inline-single-space-close t)
(setq sis-inline-tighten-head-rule 0)
(setq sis-inline-tighten-tail-rule 0)
#+end_src
Then for example final result ~中文 english 中文~:
just input expactly as the final result itself: ~中文<spc>english<spc>中文~,
that's it.
