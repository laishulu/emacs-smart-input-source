...
- ~smart-input-source-do~: combile
  ~smart-input-source-check-to-activate-overlay~ with
  ~smart-input-source-follow-context~. If no need to activate the
  overlay, then follow the context to switch input source.
