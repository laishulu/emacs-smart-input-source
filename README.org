...
5. Support ~inline english~ region:
   ...
   3) Region is closed when cursor leaves the region or ~Enter~ is pressed.
6. Input source switching rules are designed carefully.
   ...
   2) Switching considering the characters before and after the cursor, as well as crossing lines.
...
* Usage
Load the package, edit a file, and then run ~smart-input-source-mode~, if it finds no
