...
** Additional usage for none-evil user
~smart-input-source-mode~ provides three key functions to be used by end user:
- ~smart-input-source-check-to-activate-overlay~: check to activate the /inline
  english region/.
- ~smart-input-source-follow-context~: set input source smartly by following the
  context.
- ~smart-input-source-do~: do ~smart-input-source-check-to-activate-overlay~ as
  well as ~smart-input-source-follow-context~.

With ~evil~, user need not to touch those commands directly, because they are
added to ~evil~ hooks already. Without ~evil~, only
~smart-input-source-check-to-activate-overlay~ is added to hooks,
~smart-input-source-follow-context~ is not added to hooks by default.
