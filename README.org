...
5. Support ~inline english~ region:
   1) Blank after none-english characters triggers the region.
   2) In this mode, Engligh will be sticked.
   3) Region is closed when cursor leaves the region or ~Return~ is pressed.
   4) If the closed region ends with blank, none-english input source will be
      selected. If the following character is a non-english punctuation, the
...
5. 支持 inline english 区域：
   ...
   4) 关闭的区域如果是空格结尾，输入法切回非英文。后续如果输入非中文的标点符号，
      前面的空格会被自动删除。
