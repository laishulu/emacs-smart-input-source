...
- A global mode: ~sis-global-respect-mode~ to respect buffer/mode
  ...
  2. Respect ~evil~/~meow~: switch to English when leaving ~evil~/~meow~
     ~insert~ mode.
...
- A buffer local mode: ~sis-context-mode~ to switch input source smarted
  ...
  indicates when entering ~evil~ / ~meow~ insert mode.
...
8. Switch input source according to context:
   1) Pre-configured for entering evil / meow insert mode.
...
| ~sis-cursor-color-restore-hook~ | Restore editor cursor color for English (modal editors) | ~nil~                    |
|---------------------------------+---------------------------------------------------------+--------------------------|
