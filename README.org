...
- On MacOS, use Emacs other than GUI EMP, with ~macism~, no need to configure
  ...
  #+END_SRC
  Note:
  - You need to grant permission of Accessibility, check
    `https://github.com/laishulu/macism/` for detail.
  - Some Emacs "distributions" package together a number of Emacs binaries for
    ...
    the list.
    Ruby comes with macOS by default. You can find the program by opening
