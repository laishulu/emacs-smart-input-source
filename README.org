...
- On MacOS, use Emacs other than GUI EMP, with ~macism~, no need to configure
  ...
  b. Don't use ~Alacritty~ for TUI Emacs, because it can't handle input method
  well when delete composition characters, and it can't handle ~Option~ and
  ~Command~ key as well when input method is on. I just recommend ~kitty~ until
  ~Alacirtty~ fixed those long existing bugs.
